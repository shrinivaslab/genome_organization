...
class Chromosome(object):
    ...
    def add_ideal_chromosome_gamma_force(
        ...
        # Use max(d, dinit) to prevent NaN when d < dinit (OpenMM may evaluate expressions before step)
        # This ensures log and divisions are never evaluated on invalid values
        energy_expr = (
            "step(d-dinit)*(gamma1/log(max(d, dinit)) + gamma2/max(d, dinit) + gamma3/(max(d, dinit)^2))*step(dend-d)*f*step(r-lim);"
            "f=0.5*(1. + tanh(mu*(rc - r)));"
            "d=abs(idx2-idx1)"
        )
