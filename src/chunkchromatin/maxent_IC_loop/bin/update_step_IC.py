#!/usr/bin/env python3
"""
Parameter update step for MaxEnt IC loop.

The Newton update from process_IC_update.py produces both lambda_IC_tk_*.npy 
files and lambda_IC_next.npy. This script handles copying lambda_IC_next to the 
next iteration and provides diagnostics/convergence tracking.
"""

import argparse, os, json, shutil, math, subprocess
from pathlib import Path
import numpy as np
from chunkchromatin.maxent_IC_loop.bin.utils import ensure_dir, load_config, write_json, format_iter, delete_dir_if_exists
...
def main():
    ...
    # Prepare next iteration folder and copy lambda_IC_next
    it_next = it + 1
    ...
    # Also ensure lambda_IC is available for next iteration's Newton update
    next_lambda_path = iterd_next / "update" / f"lambda_IC_tk_{it_next}.npy"
    np.save(next_lambda_path, lambda_next)

    # Storage policy: delete frames from i-1
    i_minus_1 = it - 1
    if i_minus_1 >= 0:
        ...
        if old_sims.exists():
            try:
                ...
                (old_iter / "sims").mkdir(exist_ok=True)
    ...
    # Try to get name from run_manifest.json
