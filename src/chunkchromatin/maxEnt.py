...
class MaxEnt_sim_obs:
    ...
    def compute_sim_type_type_observables_streaming(self, traj_file, monomer_types, verbose=True):
        """
        Main pipeline to compute type-type observable matrix per frame.
        ...
        -------
        np.ndarray
            Array of shape (n_frames, n_types, n_types)
        """
        ...
        if verbose:
            print(f"Processing {traj_file} ({n_frames} frames, N={N})")
